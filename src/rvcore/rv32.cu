...
#define FMT_I                  \
    rd = (insn >> 7) & 0x1f;   \
    rs1 = (insn >> 15) & 0x1f; \
    uimm = (insn & 0x80000000 ? 0xfffff800 : 0) | ((insn >> 20) & 0x000007ff); \
    imm = (int32_t) uimm;
...
__device__ bool read32(uint32_t addr_, uint32_t &value, uint8_t *mem, core_status_t &cstatus)
{
    ...
    // printf("CROSIZE: %#x, ADDR: %#x, PTRLOC: %p V: %#x\n", cro_size, addr, (uint32_t *)(mem + addr), *((uint32_t *)(mem + addr)));
...
other:
    ...
    insn_masked = insn & 0xfc00707f;
    {
        ...
        {
            // RV32I srai
            DPRINTK("srai x%d, x%d, %d\n", rd, rs1, imm);
            uint32_t msr = regs[rs1] & 0x80000000;
            regs[rd] = msr ? ~(~regs[rs1] >> imm) : regs[rs1] >> imm;
            INC_PC
            return;
        }
        case 0x00005013:
...
__global__ void dumpS(REG *regfile, REG *pcfile, uint8_t *gmem, core_status_t *svec, int coreid, bool* pop_bt)
{
    int tid = blockDim.x * blockIdx.x + threadIdx.x;
    if (tid != 0)
        return;
    *pop_bt = false;
    REG *regs = regfile + coreid * NUM_OF_REGS;
    REG &pc = *(pcfile + coreid);
    if(pc == 0xf4) {
        printf("<MEMSET> (str=%#x, c=%d, n=%d)\n", regs[10], regs[11], regs[12]);
    }
    uint8_t *mem = gmem + ((uint64_t)coreid) * core_mem_size;
    // DECODE
    core_status_t cst;
